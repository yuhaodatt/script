...
local window_player = library.window("本地玩家")
local window_esp = library.window("透视")
local window_misc = library.window("其他")

window_player.toggle("本地玩家发光",false,function(val)
    flags.light = val
...
window_esp.toggle("透视门",false,function(val)
    flags.espdoors = val
...
window_esp.toggle("钥匙/工具",false,function(val)
    flags.espkeys = val
...
window_esp.toggle("物品",false,function(val)
    flags.espitems = val
...
window_esp.toggle("书",false,function(val)
    flags.espbooks = val
...
window_player.label("作者: zoophiliaphobic#6287\noh my dayyzz Tong#9966翻译",20)
window_esp.toggle("怪物",false,function(val)
    flags.esprush = val
...
window_esp.toggle("储物柜",false,function(val)
    flags.esplocker = val
...
window_esp.toggle("宝箱",false,function(val)
    flags.espchest = val
...
window_esp.toggle("钱堆",false,function(val)
    flags.espgold = val
...
window_esp.slider("透视的钱堆最小数量",5,150,5,25,function(val)
    flags.goldespvalue = val
end)

window_misc.toggle("怪物通知",false,function(val)
    flags.hintrush = val
...
window_misc.toggle("使seek不追",false,function(val)
    flags.noseek = val
...
window_misc.toggle("删除大门",false,function(val)
    flags.nogates = val
...
if screechremote then
    window_misc.toggle("screech没有伤害",false,function(val)
        flags.noscreech = val
...
window_misc.toggle("没有骷髅门",false,function(val)
...
window_misc.toggle("自动完成图书馆代码",false,function(val)
    flags.getcode = val
...
window_misc.toggle("A-000门没锁",false,function(val)
...
window_misc.toggle("搜寻附近物品",false,function(val)
...
window_misc.label("绕过反作弊，所以你不能拿起任何东西，所以只有在多人游戏或在房间区域这样做",32)

window_misc.button("绕过反作弊",function()
...
if game.ReplicatedStorage:WaitForChild("GameData"):WaitForChild("Floor").Value == "Rooms" then
    ...
    window_rooms.toggle("A90无伤",false,function(val)
        flags.noa90 = val
